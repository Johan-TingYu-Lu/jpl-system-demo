...
const SHEETS: Record<string, string> = {
    'NEW1': '1zdzsxq2j17VVjY7gpETBX0kvkrQBAnFuv6MRztkFqa0',
    'NEW2': '1GjCfmj1PiVdqITR1YuqYIf0AP5jHp5inTMoRpbcUGME',
...
async function inspect(label: string, id: string) {
    ...
    // Count students in billing sheet
    if (billingSheet) {
        const r = await sheetsApi.spreadsheets.values.get({
        ...
        if (ids.length > 0) {
            console.log(`\n  📊 計費日期表學生: ${ids.length} 人, ID ${Math.min(...ids)} ~ ${Math.max(...ids)}`);
            // Show billing count distribution
